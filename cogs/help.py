...
class help(cogcore):
    ...
    async def help(self, ctx):
        ...
        embed.add_field(name="options",
                        value="some commands must input option(s)\ne.g. /demi-nft token_id: 824 \n`824`is the option of this command.If there are more than 2 options, after entered the first one, you can press `tab` on the keyboard to switch to the next option.",
                        inline=False)
        ...
        embed0.add_field(name="GitHub", value="https://github.com/kintama48", inline=False)
        embed0.add_field(name="contact information", value="kintama#8473 | rebelschaos@gmail.com", inline=False)
        embed0.add_field(name="note",
                         value="transaction record function calls Etherscan API, OpenSea related functions calls OpenSea API.",
                         inline=False)
        ...
        embed1.add_field(name="/invite",
                         value="get bot invite link, invite bot to your server. `admin permission is required in the server you invite to`",
                         inline=False)
        ...
        embed2.add_field(name="/project", value="display real-time price of specific project. option:`project_name`",
                         inline=False)
        embed2.add_field(name="/project-history",
                         value="display history price of specific project. option:`project_name`", inline=False)
        embed2.add_field(name="/nft",
                         value="search the NFT of a specific item and a specific number. option:`contract_address` "
                               "`token_id`",
                         inline=False)
        embed2.add_field(name="/txn",
                         value="enter the address and display the transaction record. option: `eth_address`",
                         inline=False)
        embed2.add_field(name="/account",
                         value="enter the address to display ETH balance and Demi NFT balance. option: `eth_address`",
                         inline=False)
        ...
        while True:
            try:
                react, user = await self.bot.wait_for("reaction_add", timeout=60.0, check=lambda r,
